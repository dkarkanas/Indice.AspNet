...
namespace Indice.AspNetCore.Fido.Models
{
    ...
    /// </summary>
    public class PublicKeyCredential : ValidatableModel
    {
        /// <summary>
        ...
        public override ModelValidationResult Validate() {
            ...
            if (Type != "public-key") {
                errors.Add($"Type {Type} is not valid for a public key credential.");
            }
            if (!((ReadOnlySpan<byte>)WebEncoders.Base64UrlDecode(Id)).SequenceEqual(RawId)) {
                errors.Add($"Type {Type} is not valid for a public key credential.");
            }
            return errors.Any() ? ModelValidationResult.Fail(errors) : ModelValidationResult.Success();
        }
    }
